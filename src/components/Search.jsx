...
export function Search(props) {
  ...
  return (
    <div>
      ...
      {resultPosts().length > 0 && <div class="my-2">{t('search.searchLabelOne', lang)}<span class="px-2 font-bold text-skin-active">{resultPosts().length}</span>{t('search.searchLabelTwo', lang)}</div>}
        ...
        {resultPosts().map(post =>
          <>
            <a
              ...
              href={localizePath('/' + post.collection + '/' + post.slug, lang)} innerHTML={post.data.title}>
            </a>
            <div class="flex items-center">
              {post.data.date ?
                <div class="flex items-center mr-2"><i class="ri-calendar-2-fill mr-1"/>{formatDate(post.data.date, lang)}</div>
                : ''}

              {dealLabel(post.data.category).filter(item => item !== 'uncategorized').map((categoryName) => (
                <a class="flex items-center mr-2" href={localizePath("/category/" + categoryName, lang)}><i class="ri-folder-3-line mr-1"/>{categoryName}</a>
              ))}

              {dealLabel(post.data.tags).map((tagName) => (
                <a class="flex items-center mr-2" href={localizePath("/tags/" + tagName, lang)}><i class="ri-hashtag mr-1"/>{tagName}</a>
              ))}
            </div>
